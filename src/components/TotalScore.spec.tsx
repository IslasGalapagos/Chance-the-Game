...
import TotalScore, {Props} from './TotalScore';
...
describe('<TotalScore/>', () => {
  it('renders correctly', () => {
    const treeAndStyles = testRenderer.create(<TotalScore  {...props} />).toJSON();
  ...
  it('sets props.name to username', () => {
    ...
    const userName = rows.at(0).children().at(1).text().split(': ')[0];
  ...
  it('shows props.score for user and random', () => {
    const component = shallow(<TotalScore {...props} />);
    const rows = component.find('li');
    const userScore = parseInt(rows.at(0).text().split(': ')[1], 10);
    const randomScore = parseInt(rows.at(1).text().split(': ')[1], 10);
    expect(userScore).toEqual(props.score[0]);
    expect(randomScore).toEqual(props.score[1]);
  });

  it('shows coefficient for user and random if it greater than 1', () => {
    const component = shallow(<TotalScore {...props} />);
    let userCoefficient = component.find('li').at(0).find('span');
    let randomCoefficient = component.find('li').at(1).find('span');
    ...
    userCoefficient = component.find('li').at(0).find('span');
    randomCoefficient = component.find('li').at(1).find('span');
